'use client';

import { useAppContext } from '@/components/AppContextProvider';
import { useIsMobile } from '@/hooks/use-mobile';

export function StickyCta() {
  const ctx = useAppContext();
  const isMobile = useIsMobile();

  if (isMobile) return null;

  return (
    <div className="fixed inset-x-0 bottom-0 z-60 border-t border-gold/25 bg-ink/95 px-3.5 py-3 backdrop-blur-lg sm:px-7">
      <div className="mx-auto flex max-w-300 flex-wrap items-center gap-3">
        <div className="mr-auto min-w-35">
          <div className="text-[15px] text-[#F1E7D5]">
            {ctx.selectedTimeSlotId
              ? `${ctx.selectedPackage.name} · ${ctx.selectDay} ${ctx.selectTime}`
              : `${ctx.selectedPackage.name} csomag · ${ctx.selectedPackage.price}`}
          </div>
        </div>
        <a
          href="#foglalas"
          className="btn-cta min-w-[min(100%,200px)] px-5 py-3.75 text-[15px] sm:px-8"
        >
          Időpontot választok
        </a>
      </div>
    </div>
  );
}
